---
name: signin
description: "Connect Codex to Summation via the hosted MCP server. Use when the user needs to connect Summation, when Summation tools are missing or unauthenticated, or when any Summation MCP call fails with 401/403."
---
...
| **Internal** | Summation employees (`SUMMATION_PLUGIN_INTERNAL=1` in the shell that launched Codex; `ADDISON_PLUGIN_INTERNAL` still works) | Ask **environment**, then **tenant** guidance, then auth. |
