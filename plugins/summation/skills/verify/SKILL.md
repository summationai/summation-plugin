...
If `render.py` exits 2 because a material inventory item is missing or a claim is `not_reached`, repair that claim and run `accept.py` then `render.py` again. A `Source snapshot:` line is supporting only when the text after the prefix is a source-system noun phrase plus an optional extraction date, such as `Source snapshot: CRM revenue export, 2026-07-05`. A non-date number, currency, percent, KPI or count term, spelled quantity, or predicate such as active, missed, was, is, or equals makes that line material.
