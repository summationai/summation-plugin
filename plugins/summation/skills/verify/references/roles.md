...
Record `analytical_role` on every classification review. Report titles, owner identifiers, and reporting-period labels or dates are structural context. A statement that the report was prepared from a named source is supporting provenance. Keep those items outside the material denominator unless an exact clause span identifies a separate load-bearing analytical assertion whose truth changes a conclusion or customer action. This is claim-taker and coordinator judgment: Python validates the declared role/classification pair and does not classify from words, tags, values, or locations.
...
Write that object with `write_role_output.py` to `run/role-outputs/` named for this partition, then stop. Do not paste the bundle into chat. Output only `partition_id`, `occurrence_decisions`, and `clauses`. Classify every assigned inventory occurrence exactly once as `material_claim`, `supporting_provenance`, or `structural_context`; missing classification fails closed. Every occurrence decision names its exact occurrence id, classification, matching `analytical_role`, substantive reason, and clause ids. Use `load_bearing_analytical_assertion` only for a material claim. Use `structural_context` for report titles, owner identifiers, and reporting-period labels or dates. Use `supporting_provenance` for a statement that the report was prepared from a named source. If one occurrence also contains a separate load-bearing analytical assertion, isolate that assertion with an exact clause span. A nonmaterial decision has no clause ids. Structural context produces no check or card and is stripped before public serialization. Supporting provenance stays outside the material ledger.
...
Use native subagents as the primary path when the host supports them. The coordinator remains the only writer of merged run files. Materialize a read-only input JSON and an output JSON for every role run. The output JSON is a file under `run/role-outputs/`. Write it with `write_role_output.py`, then stop. Do not paste the bundle into chat. Record both digests, the allowed read paths, and the observed read paths. A read of a prior artifact, unrelated partition, evaluator control, or product checkout fails the route proof.
