...
This file covers the **MCP session** (host OAuth). The **CLI session** is separate — `sumcli auth login`, see `sumcli.md` §4. Being signed in on one does not sign in the other.
...
1. Plugin loads the bundled MCP entry (default env for external). It carries no credential — only `x-summation-client-context`, an analytics token. OAuth owns `Authorization`.
