...
The plugin's day-to-day work is MCP. **sumcli** is the scripted/automation CLI and needs **≥ 0.1.4**. Newer CLI releases are always compatible (`sumcli update` → PyPI latest). MCP work does not require sumcli.
...
Read `result.version`. Compare as dotted integers. Need **≥ 0.1.4**.
...
- Present but `< 0.1.4`: `sumcli update` if this binary is uv-managed. If update refuses (`NOT_UV_MANAGED`), upgrade with the same installer you used (brew / pip / pipx) — do not bootstrap a second copy.
- Already ≥ 0.1.4: continue. Do not pin an upper bound.
