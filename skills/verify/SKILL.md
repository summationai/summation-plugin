...
If `render.py` exits 2 because a material inventory item is missing or a claim is `not_reached`, repair that claim and run `accept.py` then `render.py` again. A `Source snapshot:` line is supporting only with positive proof of a pure source identity. The suffix must name a source object such as export, extract, file, table, dataset, warehouse, database, system, or snapshot. A source name and extraction date are optional. Example: `Source snapshot: CRM revenue export, 2026-07-05`. Every other Source snapshot line is material.
