...
If `render.py` exits 2 because a material inventory item is missing or a claim is `not_reached`, repair that claim and run `accept.py` then `render.py` again. A `Source snapshot:` line is supporting only when the entire suffix is a short source name or path that ends in a source-object noun. Allowed nouns are export, extract, file, table, dataset, warehouse, database, system, and snapshot. After that noun, only optional punctuation and one extraction date are allowed. Example: `Source snapshot: CRM revenue export, 2026-07-05`. Every other Source snapshot line is material.
