...
Before acceptance, run the same `accept.py` command shown below with `--preflight-only` and change `--out` to `$RUN/preflight.json`. For PDF, xlsx, and pptx, keep `--report-text`. Preflight returns exact repair reasons for coordinator membership, substantive supporting or structural reasons, and public numeric calculation results. The run has one total repair pass. Give the responsible role only its original input, its prior output, and the exact mechanical reasons. Do not prescribe replacement classifications, verdicts, severities, ids, labels, operands, calculations, counts, score, or action text. Rerun that failed validation once, and stop if any reason remains.
