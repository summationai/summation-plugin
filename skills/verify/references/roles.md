...
Use native subagents as the primary path when the host supports them. The coordinator remains the only writer of merged run files. Materialize a read-only input JSON and an output JSON for every role run. The output JSON is a file under `run/role-outputs/`. Write that file, then stop. Do not paste the bundle into chat. Record both digests, the allowed read paths, and the observed read paths. A read of a prior artifact, unrelated partition, evaluator control, or product checkout fails the route proof.
